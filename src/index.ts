...
import { ensureOpencodeJsonPermissions } from './opencode-json';

export const GitLoopsPlugin: Plugin = async ({ client, worktree }) => {
  ...
  try {
    ...
    // Add the cache dir to the project's opencode.json external_directory
    if (worktree) {
      try {
        await ensureOpencodeJsonPermissions(worktree, pluginConfig.cache_loc);
      } catch (permErr: any) {
        await logger.warn(
          `Failed to update opencode.json permissions: ${permErr.message || permErr}`,
        );
      }
    }
  } catch (err: any) {
    ...
    config: async (config: Config) => {
      try {
        ...
        if (cfg.agent.enabled) {
          ...
          await logger.info(
            'Gitloops agent registration skipped (agent.enabled is false)',
          );
        }
      } catch (err: any) {
        await logger.error(
